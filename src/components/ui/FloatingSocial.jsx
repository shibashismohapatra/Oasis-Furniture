...
export default function FloatingSocial() {
  ...
  return (
    <>
      ...
      <div className={`fs-wrap ${visible ? 'fs-visible' : 'fs-hidden'}`}>
        {socials.map((s, i) => (
          <div key={s.key}>
            <a
              href={s.url}
              ...
              className={`fs-btn${hovered === s.key ? ' fs-expanded' : ''}`}
              style={{ background: hovered === s.key ? s.hover : s.color }}
            ...
            {i < socials.length - 1 && (
              <div
                className="fs-divider"
                style={{ background: hovered === s.key || hovered === socials[i+1]?.key ? 'transparent' : 'rgba(255,255,255,0.18)' }}
              />
            )}
          </div>
        ))}
